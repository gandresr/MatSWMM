...
static void saveAccumRainfall(DateTime date1, int hour, int minute, int second, long v);
static void saveRainfall(DateTime date1, int hour, int minute, int second, float x,
            char isMissing);
...
//
{
    ...
    while ( k < lineLength )
    {
        ...
        switch ( fileFormat )
        {
          ...
          case NWS_ONLINE_60:
          case NWS_ONLINE_15:
              n = sscanf(&line[k], " %2d:%2d:%2d", &hour, &minute, &second);
        ...
        if ( fileFormat != NWS_ONLINE_15) second = 0;
        ...
        if ( flag1 == 'a' )
        {
            AccumStartDate = date1 + datetime_encodeTime(hour, minute, second);
        }
        else if ( flag1 == 'A' )
        {
            saveAccumRainfall(date1, hour, minute, second, v);
        }
        ...
        else
        {
            ...
            x = (float)v / 100.0f;
            if ( x > 0 || isMissing )
                saveRainfall(date1, hour, minute, second, x, isMissing);
        }
...
//
{
    ...
    for (j=1; j<=jMax; j++)
    {
        ...
        if ( x > 0 || isMissing)
        {
            saveRainfall(date1, hour, minute, 0, x, isMissing);
        }
...
//
{
    ...
    saveRainfall(date1, hour, minute, 0, x, FALSE);
    return 1;
}
...
void saveAccumRainfall(DateTime date1, int hour, int minute, int second, long v)
//
...
//
{
    ...
    date2 = date1 + datetime_encodeTime(hour, minute, second);
...
void saveRainfall(DateTime date1, int hour, int minute, int second, float x, char isMissing)
//
...
//
{
    ...
    if ( !isMissing )
    {
        seconds = 3600*hour + 60*minute + second - TimeOffset;
