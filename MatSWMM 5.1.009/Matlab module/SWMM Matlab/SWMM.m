classdef SWMM < handle
	properties (Constant)
		...
		prop = struct(...
			'SUBCATCH', struct(...
				'ID', 0, 'RPTFLAG', 1, 'GAGE', 2, 'OUTNODE', 3, 'OUTSUBCATCH', 4, ...
				'INFIL', 5, 'SUBAREA', 6, 'WIDTH', 7, 'AREA', 8, 'FRACIMPERV', 9, ...
				'SLOPE', 10, 'CURBLENGTH', 11, 'INITBUILDUP', 12, 'LANDFACTOR', 13, ...
				'GROUNDWATER', 14, 'GWLATFLOWEXPR', 15, 'GWDEEPFLOWEXPR', 16, ...
				'SNOWPACK', 17, 'LIDAREA', 18, 'RAINFALL', 19, 'EVAPLOSS', 20, ...
				'INFILLOSS', 21, 'RUNON', 22, 'OLDRUNOFF', 23, 'NEWRUNOFF', 24, ...
				'OLDSNOWDEPTH', 25, 'NEWSNOWDEPTH', 26, 'OLDQUAL', 27, 'NEWQUAL', 28, ...
				'PONDEDQUAL', 29, 'TOTALLOAD', 30), ...
			'NODE', struct('ID', 0, 'TYPE', 1, 'SUBINDEX', 2, 'RPTFLAG', 3, 'INVERTELEV', 4, ...
				'INITDEPTH', 5, 'FULLDEPTH', 6, 'SURDEPTH', 7, 'PONDEDAREA', 8, ...
				'EXTINFLOW', 9, 'DWFINFLOW', 10, 'RDIIINFLOW', 11, 'TREATMENT', 12, ...
				'DEGREE', 13, 'UPDATED', 14, 'CROWNELEV', 15, 'INFLOW', 16, 'OUTFLOW', 17, ...
				'LOSSES', 18, 'OLDVOLUME', 19, 'NEWVOLUME', 20, 'FULLVOLUME', 21, 'OVERFLOW', 22, ...
				'OLDDEPTH', 23, 'NEWDEPTH', 24, 'OLDLATFLOW', 25, 'NEWLATFLOW', 26, 'OLDQUAL', 27, ...
				'NEWQUAL', 28, 'OLDFLOWINFLOW', 29, 'OLDNETINFLOW', 30), ...
			'LINK', struct('ID', 0, 'TYPE', 1, 'SUBINDEX', 2, 'RPTFLAG', 3, 'NODE1', 4, ...
				'NODE2', 5, 'OFFSET1', 6, 'OFFSET2', 7, 'XSECT', 8, 'Q0', 9, 'QLIMIT', 10, ...
				'CLOSSINLET', 11, 'CLOSSOUTLET', 12, 'CLOSSAVG', 13, 'SEEPRATE', 14, ...
				'HASFLAPGATE', 15, 'OLDFLOW', 16, 'NEWFLOW', 17, 'OLDDEPTH', 18, 'NEWDEPTH', 19, ...
				'OLDVOLUME', 20, 'NEWVOLUME', 21, 'SURFAREA1', 22, 'SURFAREA2', 23, 'QFULL', 24, ...
				'SETTING', 25, 'TARGETSETTING', 26, 'FROUDE', 27, 'OLDQUAL', 28, 'NEWQUAL', 29, ...
				'TOTALLOAD', 30, 'FLOWCLASS', 31, 'DQDH', 32, 'DIRECTION', 33, 'BYPASSED', 34, ...
				'NORMALFLOW', 35, 'INLETCONTROL', 36));
		subprop = struct(...
			...
			'PUMP', struct('TYPE', 0, 'PUMPCURVE', 1, 'INITSETTING', 2, 'YON', 3, 'YOFF', 4, ...
				'XMIN', 5, 'XMAX', 6), ...
			'ORIFICE', struct('TYPE', 0, 'SHAPE', 1, 'CDISCH', 2, 'ORATE', 3, 'CORIF', 4, ...
				'HCRIT', 5, 'CWEIR', 6, 'LENGTH', 7, 'SURFAREA', 8), ...
	...
	%
	% swmm.get(swmmType)
	% swmm.get(swmmType, swmmProp)
	% swmm.get(swmmType, swmmSubType, swmmProp)
	% swmm.get(ID, swmmType, swmmProp)
	% swmm.get(IDs, swmmType, swmmProp)
	% swmm.get(ID, swmmType, swmmSubType, swmmProp)
	% swmm.get(IDs, swmmType, swmmSubType, swmmProp)
	%
	...
	% IDs: cell with identifications of several objects.
		...
		if getAll
			...
			error = calllib('swmm5', 'swmm_get_all', idsPtr, swmmType, swmmSubType);
			if error ~= 0
				obj.throw_error(error);
			end
